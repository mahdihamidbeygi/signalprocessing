...
def wavelet(data,times,dt,PGV_pos,method):
  ...
  '''
  power,period = wave_analy(data,times,dt,method)
  ...
  return tp,max_power_tp,max_power_pos,power,period
...
def pgv_pos_finder(data):
  ...
  PGV position finder'''
  data = np.abs(data)   
  return np.max(data), np.where(data == np.max(data))[0][0]
...
def analysis(data,times,dt,PGV,PGV_pos,method,order = -1):
  ...
  if method == 'Ricker':
    # RICKER ANALYSIS
    # Wavelet Analysis
    pgv_tp, max_tp, max_tp_pos, power, period = wavelet(data,times,dt,PGV_pos,'DOG')
    ...
    if ((max_e/total_e)+(max_spec_e/spec_e))/2 >= 0.30 and max_e >= pgv_e*1.1 and max_spec_e >= pgv_spec_e*1.1 and abs(EMAX_GV) >= 25 and abs(PGV_pos - max_tp_pos)*dt > pgv_tp/4:
    ...
    elif ((pgv_e/total_e) + (pgv_spec_e/spec_e))/2 >= 0.30:
      is_pulse_dz = 1
      Tp = pgv_tp
      ...
      late = float('NaN')
      e_change = 0
      data_ricker = []; power = [];period = []; 
  elif method == 'Morlet':
    # MORLET ANALYSIS
    # Wavelet Analysis
    pgv_tp, max_tp, max_tp_pos, power, period = wavelet(data,times,dt,PGV_pos,'MORLET')
    ...
    if ((max_e/total_e)+(max_spec_e/spec_e))/2 >= 0.30 and max_e >= pgv_e*1.1 and max_spec_e >= pgv_spec_e*1.1 and abs(EMAX_GV) >= 25 and abs(PGV_pos - max_tp_pos)*dt > pgv_tp/4:
      ...
      e_change = float('NaN')
      data_ricker = []; power = [];period = [];      
  return is_pulse_dz, Tp, p_e, spec_p_e, late, e_change, data_ricker
...
def vel_wf_detector(st, scale = 1, zero_adding = False):
  ...
  #  if abs(PGV) >= 30:
    if PGV != 0:
        ...
        is_pulse_dz, Tp, p_e, spec_p_e, late, e_change, data_ricker = analysis(data,times,dt,PGV,PGV_pos,method = 'Ricker')
        ...
        is_pulse_dz, Tp, p_e, spec_p_e, late, e_change, data_ricker = analysis(data,times,dt,PGV,PGV_pos,method = 'Morlet',order = 3)
        r1.append(is_pulse_dz);r2.append(Tp);r3.append(p_e);r4.append(spec_p_e);r5.append(late);r6.append(e_change);
        r7.append(data_ricker);#r8.append(power);r9.append(period);#r10.append(sig95);r11.append(coi);r12.append(global_ws);r13.append(global_signif)
        
        is_pulse_dz, Tp, p_e, spec_p_e, late, e_change, data_ricker = analysis(data,times,dt,PGV,PGV_pos,method = 'Morlet',order = 4)
        r1.append(is_pulse_dz);r2.append(Tp);r3.append(p_e);r4.append(spec_p_e);r5.append(late);r6.append(e_change);
        r7.append(data_ricker);#r8.append(power);r9.append(period);#r10.append(sig95);r11.append(coi);r12.append(global_ws);r13.append(global_signif)
...
#        spec_p_e = r4[amax]
      ...
      #     _, _, p_e, spec_p_e, _, _, _, _,_, _, _, _, _ = analysis(data,times,dt,PGV,PGV_pos,method = 'Ricker')
        ...
        fit_wavelet = False
    return Tp, is_pulse_dz, fit_wavelet
